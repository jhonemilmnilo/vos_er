// lib/modules/approvals/leave/leave_view.dart
import "dart:async";
...
class _LeaveApprovalViewState extends ConsumerState<LeaveApprovalView> {
  ...
  LeaveFilter _selectedFilter = LeaveFilter.pending;
  String _query = "";
  ...
  void initState() {
    super.initState();
    _repo = LeaveRepository(ref.read(apiClientProvider));
  ...
  Future<void> _loadMore({bool initial = false}) async {
    ...
    try {
      ...
      final page = await _repo.fetchLeaveApprovalsPaged(
        status: status,
        search: q,
        limit: _pageSize,
        offset: _offset,
  ...
  Future<void> _showFilterMenu() async {
    ...
    final selected = await showModalBottomSheet<LeaveFilter>(
      context: context,
      useSafeArea: true,
      showDragHandle: true,
      ...
      builder: (ctx) {
        ...
        final visible = LeaveFilter.values.toList();
        return Material(
  ...
  Future<void> _openApprovalModal(LeaveApprovalHeader row) async {
    // Only pending is actionable
    if (!row.isPending) return;
    HapticFeedback.selectionClick();

    final outcome = await showModalBottomSheet<LeaveApproveOutcome?>(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
      backgroundColor: Colors.transparent,
      builder: (_) => LeaveApprovalSheet(header: row),
    );
    ...
    final msg = outcome.newStatus == LeaveStatus.approved
        ? "Approved leave #${outcome.leaveId}."
        : "Rejected leave #${outcome.leaveId}.";
...
class _LeaveCard extends StatelessWidget {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                            _formatSimpleDate(header.requestDate),
                            style: TextStyle(
                              color: cs.onSurfaceVariant,
                              fontSize: 11,
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                child: Text(
                                  header.leaveType.label.toUpperCase(),
                                  style: TextStyle(
                                    fontSize: 10,
                              ...
                              const SizedBox(width: 8),
                              Text(
                                _formatTotalDays(header.totalDaysLabel),
                ...
                // Reason / Period
                if (header.reason.trim().isNotEmpty) ...[
  ...
  static Color _statusColor(LeaveStatus s, ColorScheme cs) {
    switch (s) {
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.beach_access_rounded, size: 48, color: cs.onSurfaceVariant),
            ),
            const SizedBox(height: 24),
            Text(
              query.trim().isEmpty ? "No Leave Requests" : "No results for \"$query\"",
              style: TextStyle(fontWeight: FontWeight.w800, color: cs.onSurface, fontSize: 18),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 8),
            Text(
              "Looks like everyone is hard at work!\nOr no one is planning a vacation yet.",
